  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        if (!firstSheetName) {
          return NextResponse.json({ error: `No worksheets found in file: ${file.name}` }, { status: 400 })
        }
        ...
        if (!headers || headers.length === 0) {
          return NextResponse.json({ error: `No headers found in file: ${file.name}` }, { status: 400 })
        }
    ...
    const worksheet = XLSX.utils.json_to_sheet(consolidatedData, { header: finalColumns })

    // Auto-size columns
    const colWidths = finalColumns.map((col) => {
      const maxLength = Math.max(col.length, ...consolidatedData.map((row) => String(row[col] || "").length))
    ...
    return new NextResponse(excelBuffer, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="consolidated-excel-${new Date().toISOString().split("T")[0]}.xlsx"`,
      },
    })
  } catch (error) {
